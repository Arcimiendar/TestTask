...
def test_sub_functor():
    ...
    assert r == -1, f'"Sub" functor is not working, got {r}, expectet -1'


def test_mul_functor():
    r = functors[OperationOptions.mul](5, 6)
    assert r == 30, f'"Mul" functor is not working, got {r}, expectet 30'


def test_div_functor():
    r = functors[OperationOptions.div](1, 2)
    assert r == 0.5, f'"Div" functor is not working, got {r}, expectet 0.5'
